...
interface BillingWorkspaceProps {
  layout: React.ComponentType<BillingLayoutProps>;
  activeRoute: string;
  title?: string;
  subtitle?: string;
}
...
function buildGstBreakup(items: Array<Pick<ReceiptItem, 'gstRate' | 'gstAmount' | 'lineTotal'>>) {
...
export default function BillingWorkspace({
  ...
  const handleBarcodeScan = useCallback(async (code: string) => {
    ...
    try {
      ...
      await addToCart({
        ...data.product,
        availableQty: Number(data.product.scannableQty ?? data.product.availableQty ?? 0),
      });
    } catch {
  ...
  const syncReservations = useCallback(async (
    ...
    if (!shopId || !user?.id) {
      return { ok: true as const, cart: nextCart };
    }
    try {
      const response = await fetch('/api/billing/reservations', {
        method: 'POST',
        credentials: 'include',
        ...
        body: JSON.stringify({
          ...
          items: nextCart.map(item => ({ productId: item.product._id, qty: item.qty })),
        }),
      });

      const payload = await response.json().catch(() => null) as {
        error?: string;
        reservations?: Array<{ productId: string; scannableQty: number }>;
      } | null;

      if (!response.ok) {
        if (!options?.silent) {
          setScanError(payload?.error ?? 'This product is currently locked on another counter.');
        }
        return { ok: false as const, cart: cartRef.current };
      }

      const reservationMap = new Map((payload?.reservations ?? []).map(row => [row.productId, row.scannableQty]));
      const hydratedCart = nextCart.map(item => ({
        ...item,
        product: {
          ...item.product,
          availableQty: reservationMap.get(item.product._id) ?? item.product.availableQty,
        },
      }));

      return { ok: true as const, cart: hydratedCart };
    } catch {
      if (!options?.silent) {
        setScanError('Failed to sync scan lock. Please try again.');
      }
      return { ok: false as const, cart: cartRef.current };
    }
  }, [shopId, user?.email, user?.id, user?.name]);

  async function addToCart(product: ScannedProduct) {
    const effectiveAvailableQty = Number(product.scannableQty ?? product.availableQty ?? 0);
    const existing = cartRef.current.find(c => c.product._id === product._id);

    if (existing) {
      if (existing.qty + 1 > effectiveAvailableQty) {
        setScanError(`Only ${effectiveAvailableQty} unit(s) of "${product.name}" can be scanned right now.`);
        return;
      }

      const nextCart = cartRef.current.map(item => item.product._id === product._id
        ? { ...item, qty: item.qty + 1, product: { ...item.product, availableQty: effectiveAvailableQty } }
        : item);
      const result = await syncReservations(nextCart);
      if (!result.ok) return;
      setScanError('');
      setCart(result.cart);
      return;
    ...
    if (effectiveAvailableQty < 1) {
      setScanError(`"${product.name}" is already locked on another counter or out of stock.`);
      return;
    }
  ...
  async function updateQty(productId: string, qty: number) {
    ...
    const existing = cartRef.current.find(item => item.product._id === productId);
    if (!existing) return;
    if (qty > existing.product.availableQty) {
      setScanError(`Only ${existing.product.availableQty} unit(s) of "${existing.product.name}" can be held right now.`);
      return;
    }

    const nextCart = cartRef.current.map(item => item.product._id === productId ? { ...item, qty } : item);
    const result = await syncReservations(nextCart);
    if (!result.ok) return;
    setScanError('');
    setCart(result.cart);
  ...
  async function removeFromCart(productId: string) {
    const nextCart = cartRef.current.filter(item => item.product._id !== productId);
    const result = await syncReservations(nextCart, { silent: true });
    if (!result.ok) return;
    setCart(result.cart);
  }

  async function clearCart() {
    const result = await syncReservations([], { silent: true });
    if (result.ok) {
      setCart([]);
    }
    setScanError('');
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/products?shopId=${shopId}&currentUserId=${encodeURIComponent(user?.id ?? '')}&search=${encodeURIComponent(val.trim())}&limit=10`,
          { credentials: 'include' },
        );
        const data = await res.json();
        if (res.ok) {
          setSearchResults((data.products ?? []).map((product: ScannedProduct & { scannableQty?: number }) => ({
            ...product,
            availableQty: Number(product.scannableQty ?? product.availableQty ?? 0),
          })));
  ...
  async function selectSearchResult(p: ScannedProduct) {
    await addToCart(p);
    setSearchQuery('');
    setSearchResults([]);
    setShowSearchDrop(false);
  ...
  const gstAmount = parseFloat(cart.reduce((sum, item) => {
    const lineTotal = item.product.price * item.qty;
    const rate = Math.max(0, Math.min(100, Number(item.product.saleGstRate ?? 0)));
    const lineGst = rate > 0 ? (lineTotal * rate) / (100 + rate) : 0;
    return sum + lineGst;
  }, 0).toFixed(2));
  const subtotal = parseFloat((cartTotal - gstAmount).toFixed(2));
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      await syncReservations([], { silent: true });
  ...
  function printReceipt() {
    ...
    const gstBreakup = buildGstBreakup(receipt.items);
    ...
    const itemRows = receipt.items.map((item, i) => `
      <tr>
        ...
        <td class="l">
          ...
          <div class="item-meta">${item.hsnCode ? `HSN: ${item.hsnCode}` : ''}${item.hsnCode && item.gstAmount > 0 ? ' · ' : ''}${item.gstAmount > 0 ? `${item.gstRate}% GST incl. ${fmt(item.gstAmount)}` : ''}</div>
        </td>
...
<body>
  ...
  <div class="totals-wrap">
    <div class="totals-box">
      <div class="t-row"><span>Taxable Subtotal</span><span>${fmt(receipt.subtotal)}</span></div>
      ${receipt.gstAmount > 0
        ? `<div class="t-gst"><span>${hasMixedGstRates ? 'Included GST (Total)' : gstSummaryLabel(receipt.gstRate)}</span><span>${fmt(receipt.gstAmount)}</span></div>`
        : ''
      }
      ${gstBreakup.length > 0
  ...
  useEffect(() => {
    if (!shopId || !user?.id || cart.length === 0) return;

    const intervalId = window.setInterval(() => {
      void syncReservations(cartRef.current, { silent: true });
    }, 60 * 1000);

    return () => window.clearInterval(intervalId);
  }, [cart.length, shopId, syncReservations, user?.id]);
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      void fetch('/api/billing/reservations', {
        method: 'POST',
        credentials: 'include',
        keepalive: true,
        headers: { 'Content-Type': 'application/json' },
            ...
            <div className="px-8 pt-5 pb-7">
                  ...
                  {receipt.items.map((item, i) => (
                    <tr key={i} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="py-3 px-2">
                        ...
                        {(item.hsnCode || item.gstAmount > 0) && (
                          <div className="text-[11px] text-slate-500 mt-0.5">
                            {item.hsnCode ? <span>HSN: {item.hsnCode}</span> : null}
                            {item.hsnCode && item.gstAmount > 0 ? <span>&nbsp;·&nbsp;</span> : null}
                            {item.gstAmount > 0 ? <span>{item.gstRate}% GST incl. {fmt(item.gstAmount)}</span> : null}
                          </div>
                        )}
                      </td>
              ...
              <div className="mt-4 ml-auto w-72 space-y-1.5">
                <div className="flex justify-between text-sm text-slate-500 border-t border-dashed border-slate-200 pt-3">
                  <span>Taxable Subtotal</span><span>{fmt(receipt.subtotal)}</span>
                </div>
                {(receipt.gstAmount ?? 0) > 0 ? (
                  <div className="flex justify-between text-sm text-amber-700 bg-amber-50 rounded-lg px-2 py-1">
                    <span>{hasMixedGstRates ? 'Included GST (Total)' : gstSummaryLabel(receipt.gstRate)}</span><span>{fmt(receipt.gstAmount)}</span>
                  </div>
                ) : null}
                {gstBreakup.length > 0 && (
              ...
              {showSearchDrop && searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-30 overflow-hidden max-h-72 overflow-y-auto">
                  {searchResults.map(p => (
                    <button
                      key={p._id}
                      onClick={() => void selectSearchResult(p)}
                    ...
                    >
                      ...
                      <div className="text-right ml-4">
                        ...
                        <div className={`text-xs font-medium ${p.availableQty === 0 ? 'text-red-500' : p.availableQty <= 20 ? 'text-amber-500' : 'text-emerald-600'}`}>
                          {p.availableQty === 0 ? 'Locked / out of stock' : `${p.availableQty} ready to scan`}
                        </div>
                      </div>
                    </button>
                  ))}
                ...
                {cart.map(item => (
                  <div key={item.product._id} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => void updateQty(item.product._id, item.qty - 1)}
                      ...
                      <input
                        ...
                        value={item.qty}
                        onChange={e => void updateQty(item.product._id, Math.max(1, parseInt(e.target.value) || 1))}
                    ...
                    {/* Remove */}
                    <button
                      onClick={() => void removeFromCart(item.product._id)}
        ...
        <div className="lg:col-span-2 space-y-4 lg:sticky lg:top-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <div className="border-t border-dashed border-slate-200 pt-3 space-y-2">
              ...
              {gstAmount > 0 && (
                <div className="flex justify-between text-sm text-amber-700 bg-amber-50 rounded-lg px-2 py-1">
                  <span>Included GST (already in price)</span>
