...
export default function ShopAdminReportsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/reports/shop?shopId=${shopId}&range=${dateRange}`, { credentials: 'include' });
  ...
  const handleExport = async (type: 'inventory' | 'stock') => {
    if (!shopId) return;
    setExporting(type);
    try {
      const res = await fetch(`/api/reports/shop?shopId=${shopId}&range=${dateRange}&export=${type}`, { credentials: 'include' });
      if (!res.ok) throw new Error();
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2.5 mb-0.5">
              <div className="w-7 h-7 rounded-lg bg-emerald-600 flex items-center justify-center">
                <Icon name="ChartBarIcon" size={14} className="text-white" />
              </div>
              <h1 className="text-2xl font-bold text-slate-900">Reports</h1>
            </div>
            <p className="text-slate-500 text-sm">{shopName} · Analytics & Performance</p>
          </div>
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <div className="flex gap-1 p-0.5 bg-slate-100 rounded-lg">
              {(['7d','30d','90d','1y'] as DateRange[]).map(r => (
                <button key={r} onClick={() => setDateRange(r)}
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${dateRange === r ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                  {r}
                </button>
              ))}
            </div>
            ...
            <button onClick={() => handleExport('inventory')} disabled={loading || !!exporting}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {[
            { label: 'Catalog Value',   value: loading ? null : fmtMoney(kpi?.catalogValue ?? 0),   icon: 'BanknotesIcon',             color: 'bg-emerald-50 text-emerald-600',  border: 'border-emerald-100', sub: `${kpi?.totalProducts ?? 0} products` },
            ...
            { label: 'Total SKUs',      value: loading ? null : String(kpi?.totalProducts ?? 0),      icon: 'TagIcon',                   color: 'bg-indigo-50 text-indigo-600',    border: 'border-indigo-100',  sub: 'unique products' },
            { label: 'Available',       value: loading ? null : fmtNum(kpi?.availableUnits ?? 0),     icon: 'CheckCircleIcon',           color: 'bg-teal-50 text-teal-600',        border: 'border-teal-100',    sub: 'units in stock' },
            { label: 'Low Stock',       value: loading ? null : String(kpi?.lowStock ?? 0),           icon: 'ExclamationTriangleIcon',   color: 'bg-amber-50 text-amber-600',      border: 'border-amber-100',   sub: '≤ 20 units' },
            { label: 'Out of Stock',    value: loading ? null : String(kpi?.outOfStock ?? 0),         icon: 'XCircleIcon',               color: 'bg-red-50 text-red-600',          border: 'border-red-100',     sub: '0 units left' },
          ].map(card => (
            <div key={card.label} className={`bg-white rounded-xl border ${card.border} shadow-sm p-4`}>
              ...
              {loading
                ? <><Skeleton className="h-6 w-16 mb-1" /><Skeleton className="h-3 w-20 mt-1" /></>
                : <>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <p className="text-xs text-slate-400 mb-4">Generate and export detailed shop reports as CSV</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { id: 'inventory' as const, label: 'Stock Summary',    icon: 'ClipboardDocumentListIcon', color: 'text-emerald-600', bg: 'bg-emerald-50', desc: 'Current stock levels and alerts' },
              ...
              { id: null,                 label: 'Restock History',  icon: 'TruckIcon',                 color: 'text-violet-600',  bg: 'bg-violet-50',  desc: 'All restock requests and fulfillments' },
              { id: null,                 label: 'Category Report',  icon: 'TagIcon',                   color: 'text-indigo-600',  bg: 'bg-indigo-50',  desc: 'Product performance by category' },
            ].map((r, i) => (
              <button key={i}
                onClick={() => r.id && handleExport(r.id)}
                disabled={loading || !!exporting || !r.id}
                className="flex items-start gap-3 p-4 rounded-xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all text-left group disabled:opacity-60 disabled:cursor-not-allowed">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-700 group-hover:text-emerald-700 transition-colors">{r.label}</p>
        ...
        {/* ── Charts Row 1: Movement + Stock Status Pie ─────────────────── */}
              ...
              {loading ? (
                ...
                <>
                  <div className="flex items-center gap-5 mb-4">
                    {[{ color: '#10b981', label: 'Stock In' }, { color: '#ef4444', label: 'Stock Out' }].map(l => (
        ...
        {/* ── Movement Trend Line Chart (90d / 1y only) ─────────────────── */}
        {!useDaily && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-slate-100">
        ...
        {/* ── Bottom Row: Top Products + Movement by Type + Recent Activity ── */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Highest stock value in your inventory</p>
            </div>
                  ...
                  : (data?.topProducts ?? []).map((p, i) => {
                      ...
                      return (
                        <div key={p.sku} className="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50/50 transition-colors">
                          <div className="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center shrink-0 text-xs font-bold text-emerald-600">
                            {i + 1}
                          </div>
