...
} from '@/lib/barcodeLabelSizes';
import { qzConnect, qzDisconnect, qzIsConnected, qzGetPrinters, qzPrintLabels } from '@/lib/qzTray';
...
function withBarcodeLabelSize(url: string, size: BarcodeLabelSizeKey) {
...
export default function ShopAdminProductsPage() {
  ...
  const [barcodeLabelSize, setBarcodeLabelSize] = useState<BarcodeLabelSizeKey>(
    DEFAULT_BARCODE_LABEL_SIZE
  );
  ...
  // ── QZ direct print ───────────────────────────────────────────────────────
  async function handlePrintLabels(baseUrl: string, size: BarcodeLabelSizeKey) {
    setIsPrinting(true);
    try {
      const printUrl = withBarcodeLabelSize(baseUrl, size);
      if (qzStatus === 'connected' && qzPrinter) {
        // ── Path A: QZ Tray direct print (no dialog) ──────────────────────
        const jsonUrl = `${printUrl}${printUrl.includes('?') ? '&' : '?'}format=json`;
        ...
        const labelSize = BARCODE_LABEL_SIZES[size];
        await qzPrintLabels(
          qzPrinter,
          data.labels.map((l) => l.pngBase64),
          {
      ...
      window.open(withBarcodeLabelSize(baseUrl, size), '_blank');
    } finally {
      setIsPrinting(false);
    }
  }
    ...
    () =>
      Array.from(
        new Set([
          ...
          ...products
            .map((product) => product.sourceState?.trim())
    ...
    return Array.from(
      new Set(
        products
          .filter(
            (product) =>
              !selectedState || (product.sourceState ?? '').trim().toLowerCase() === selectedState
          )
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      setStats(
        data.stats ?? { totalProducts: 0, totalUnits: 0, availableUnits: 0, catalogValue: 0 }
      );
    } catch (err) {
  ...
  const validateForm = (): boolean => {
    ...
    if (form.purchasePrice.trim() && (purchasePrice === null || purchasePrice < 0))
      errors.purchasePrice = 'Enter a valid purchasing price.';
    const tax = toOptionalNumber(form.tax);
    if (form.tax.trim() && (tax === null || tax < 0)) errors.tax = 'Enter a valid tax amount.';
    if (form.saleGstRate.trim() && toOptionalRate(form.saleGstRate) === null)
      errors.saleGstRate = 'Enter a valid GST percentage.';
    const transportationCost = toOptionalNumber(form.transportationCost);
    if (form.transportationCost.trim() && (transportationCost === null || transportationCost < 0))
      errors.transportationCost = 'Enter a valid transportation cost.';
    if (form.lowStockAlertQty.trim() && toOptionalThreshold(form.lowStockAlertQty) === null)
      errors.lowStockAlertQty = 'Enter a valid alert quantity.';
    if (form.skuMode === 'manual' && !form.sku.trim())
      errors.sku = 'Enter a SKU or switch to Auto.';
  ...
  const validateEdit = (): boolean => {
    ...
    if (editState.saleGstRate.trim() && toOptionalRate(editState.saleGstRate) === null)
      errors.saleGstRate = 'Invalid.';
    ...
    if (
      editState.lowStockAlertQty.trim() &&
      toOptionalThreshold(editState.lowStockAlertQty) === null
    )
      errors.lowStockAlertQty = 'Invalid.';
  ...
  const SortIcon = ({ col }: { col: SortKey }) => (
    <span className="ml-1 inline-flex flex-col gap-0">
      <Icon
        name="ChevronUpIcon"
        size={10}
        className={sortKey === col && sortDir === 'asc' ? 'text-emerald-600' : 'text-slate-300'}
      />
      <Icon
        name="ChevronDownIcon"
        size={10}
        className={sortKey === col && sortDir === 'desc' ? 'text-emerald-600' : 'text-slate-300'}
      />
    </span>
  );
  ...
  const lowStockCount = useMemo(
    () =>
      products.filter(
        (p) =>
          p.availableQty > 0 && p.availableQty <= (p.lowStockAlertQty ?? defaultLowStockThreshold)
      ).length,
    [defaultLowStockThreshold, products]
  );
  ...
  const expiringSoonCount = useMemo(
    () =>
      products.filter((p) => {
        ...
        return d >= todayStart && d <= now30;
      }).length,
    [products, now30, todayStart]
  );
  ...
  const getExpiryBadge = (expiryDate?: string | null) => {
    ...
    if (d <= now30)
      return {
        label: `${Math.ceil((d.getTime() - todayStart.getTime()) / 86400000)}d`,
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            ...
            {
              label: 'Stock Value',
              value: fmtValue(stats.catalogValue),
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <div
                  className={`w-8 h-8 rounded-lg bg-${card.color}-50 flex items-center justify-center`}
                >
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50/60">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Fill in the details below — product will be added to{' '}
                <span className="font-semibold text-emerald-600">{shopName}</span>
              </p>
            </div>
          </div>
          ...
          <div className="px-6 py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                      ...
                      {nameSuggestions.map((s) => (
                        ...
                        >
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-slate-800 group-hover:text-emerald-700 truncate">
                              {s.name}
                            </p>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  Unit Price (₹, GST Included) <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  min="0"
                ...
                />
                {formErrors.price && (
                  <p className="mt-1 text-[11px] text-red-500">{formErrors.price}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {formErrors.quantity && (
                  <p className="mt-1 text-[11px] text-red-500">{formErrors.quantity}</p>
                )}
              </div>
            </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  <span className="ml-2 inline-flex rounded-lg overflow-hidden border border-slate-200 text-[10px] font-semibold">
                    <button
                      ...
                      className={`px-2 py-0.5 transition-colors ${form.skuMode === 'auto' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
                    >
                      Auto
                    </button>
                    <button
                      ...
                      className={`px-2 py-0.5 transition-colors ${form.skuMode === 'manual' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
                    >
                      Manual
                    </button>
                  </span>
                ...
                {form.skuMode === 'auto' ? (
                  ...
                  <>
                    ...
                    />
                    {formErrors.sku && (
                      <p className="mt-1 text-[11px] text-red-500">{formErrors.sku}</p>
                    )}
                  </>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                />
                {formErrors.gauge && (
                  <p className="mt-1 text-[11px] text-red-500">{formErrors.gauge}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {formErrors.weight && (
                  <p className="mt-1 text-[11px] text-red-500">{formErrors.weight}</p>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  Source State <span className="text-slate-400 font-normal">(optional)</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  Source District <span className="text-slate-400 font-normal">(optional)</span>
                </label>
                <input
                  type="text"
            ...
            <div className="bg-slate-50/70 border border-slate-200 rounded-2xl p-4 mb-5">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Optional for shop admin — superadmin can complete missing cost details later
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {formErrors.purchasePrice && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.purchasePrice}</p>
                  )}
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                    Purchasing Date <span className="text-slate-400 font-normal">(optional)</span>
                  </label>
                  <input
                    type="date"
                  ...
                  {formErrors.purchaseDate && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.purchaseDate}</p>
                  )}
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                    Tax (₹) <span className="text-slate-400 font-normal">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                  ...
                  />
                  {formErrors.tax && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.tax}</p>
                  )}
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                    Included GST (%) <span className="text-slate-400 font-normal">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                  ...
                  />
                  {formErrors.saleGstRate && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.saleGstRate}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {formErrors.transportationCost && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.transportationCost}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {formErrors.lowStockAlertQty && (
                    <p className="mt-1 text-[11px] text-red-500">{formErrors.lowStockAlertQty}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  <span className="ml-1.5 text-[10px] font-normal text-slate-400">
                    (optional — skip for electronics, etc.)
                  </span>
                </label>
                <input
                  type="date"
                ...
                {form.expiryDate && new Date(form.expiryDate) < new Date() && (
                  <p className="mt-1 text-[11px] text-red-500">
                    Date is in the past — product may already be expired.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
          <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <button
              ...
              disabled={isLoading}
              className="p-2 rounded-lg border border-slate-200 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all disabled:opacity-40"
              title="Refresh"
            >
              ...
              <tbody className="divide-y divide-slate-50">
                {isLoading ? (
                    ...
                    <tr key={`sk-${i}`} className="animate-pulse">
                      {Array.from({ length: 8 }).map((__, j) => (
                        <td key={j} className="px-4 py-3">
                          <div className="h-3 bg-slate-100 rounded w-full max-w-[120px]" />
                        </td>
                      ))}
                    </tr>
                  ))
                  ...
                  products.map((product) => {
                    ...
                    const isLowStock =
                      product.availableQty > 0 && product.availableQty <= effectiveThreshold;
                    const stockColor =
                    ...
                    return (
                      <tr key={product._id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm font-semibold text-slate-700 whitespace-nowrap">
                          ...
                          <div className="mt-0.5 text-[10px] text-slate-400 font-normal">
                            Included GST: {(product.saleGstRate ?? 0).toFixed(2)}%
                          </div>
                          {product.hsnCode ? (
                            <div className="mt-0.5 text-[10px] text-slate-400 font-normal">
                              HSN: {product.hsnCode}
                          ...
                          {product.sourceState || product.sourceDistrict ? (
                            <div className="mt-0.5 text-[10px] text-slate-400 font-normal">
                              Source:{' '}
                              {[product.sourceDistrict, product.sourceState]
                                .filter(Boolean)
                                .join(', ')}
                            </div>
                          ) : null}
                            ...
                            <button
                              onClick={() =>
                                setPrintTarget({
                                  ...
                                  price: product.price,
                                  barcodePrintUrl: `/api/products/${product._id}/barcodes`,
                                  restocked: false,
                                })
                              }
                              className="p-1.5 rounded-lg hover:bg-emerald-50 text-slate-400 hover:text-emerald-600 transition-all"
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50/50">
            <p className="text-xs text-slate-400">
              ...
              of <span className="font-medium text-slate-600">{totalCount.toLocaleString()}</span>{' '}
              products
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                ...
                <div>
                  <h3 className="text-base font-semibold text-slate-800">
                    {printTarget.restocked
                      ? 'Stock Updated — Print New Barcodes'
                      : 'Product Added — Print Barcodes'}
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    {printTarget.restocked
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                ...
                <p className="text-sm text-emerald-700">
                  {printTarget.restocked ? (
                    <>
                      <span className="font-bold">{printTarget.qty}</span> new label
                      {printTarget.qty !== 1 ? 's' : ''} for units just added.
                      <br />
                      <span className="text-emerald-500 text-xs">
                        ({printTarget.totalQty} total units in stock)
                      </span>
                    </>
                  ) : (
                    <>
                      <span className="font-bold">{printTarget.qty}</span> barcode label
                      {printTarget.qty !== 1 ? 's' : ''} will be generated — one per unit.
                    </>
                  )}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {BARCODE_LABEL_SIZE_OPTIONS.map((option) => {
                    const active = barcodeLabelSize === option.value;
                    return (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => setBarcodeLabelSize(option.value)}
                      ...
                      >
                        ...
                        <span className="block text-[11px] text-slate-500">{option.sizeText}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              >
                ...
                {qzStatus === 'connected' && qzPrinters.length > 0 && (
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-emerald-800">Printer</label>
                    <select
                      value={qzPrinter}
                ...
                {qzStatus === 'connected' && qzPrinters.length === 0 && (
                  <p className="text-xs text-amber-700">
                    No printers found. Make sure your Shreyans Barcode Printer is connected and its
                    driver is installed.
                  </p>
                )}
              </div>
              ...
              <p className="text-xs text-slate-400">
                Each label contains a unique Code 128 barcode. Choose a size that fits the product
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50">
              ...
              <button
                onClick={() => handlePrintLabels(printTarget.barcodePrintUrl, barcodeLabelSize)}
                disabled={
                  isPrinting ||
                  qzStatus === 'connecting' ||
                  (qzStatus === 'connected' && !qzPrinter)
                }
              ...
              >
                {isPrinting ? (
                  ...
                  <>
                    ...
                    Print Direct — {printTarget.qty} Label{printTarget.qty !== 1 ? 's' : ''}
                  </>
                ) : (
                  <>
                    <Icon name="PrinterIcon" size={15} className="text-white" />
                    Print {printTarget.qty} Label{printTarget.qty !== 1 ? 's' : ''} (via dialog)
                  </>
                )}
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                />
                {editErrors.name && (
                  <p className="mt-1 text-[11px] text-red-500">{editErrors.name}</p>
                )}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {editErrors.price && (
                    <p className="mt-1 text-[11px] text-red-500">{editErrors.price}</p>
                  )}
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  <input
                    ...
                    value={editState.sku}
                    onChange={(e) =>
                      setEditState((s) => ({ ...s, sku: e.target.value.toUpperCase() }))
                    }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {editErrors.gauge && (
                    <p className="mt-1 text-[11px] text-red-500">{editErrors.gauge}</p>
                  )}
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  />
                  {editErrors.weight && (
                    <p className="mt-1 text-[11px] text-red-500">{editErrors.weight}</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {editErrors.totalQty && (
                    <p className="mt-1 text-[11px] text-red-500">{editErrors.totalQty}</p>
                  )}
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  />
                  {editErrors.availableQty && (
                    <p className="mt-1 text-[11px] text-red-500">{editErrors.availableQty}</p>
                  )}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                      Purchasing Price (₹) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                      ...
                      onChange={(e) =>
                        setEditState((s) => ({ ...s, purchasePrice: e.target.value }))
                      }
                      className={`${inputBase} ${editErrors.purchasePrice ? 'border-red-300' : ''}`}
                    />
                    {editErrors.purchasePrice && (
                      <p className="mt-1 text-[11px] text-red-500">{editErrors.purchasePrice}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                      Purchasing Date <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="date"
                      value={editState.purchaseDate}
                      onChange={(e) =>
                        setEditState((s) => ({ ...s, purchaseDate: e.target.value }))
                      }
                      className={`${inputBase} ${editErrors.purchaseDate ? 'border-red-300' : ''}`}
                    />
                    {editErrors.purchaseDate && (
                      <p className="mt-1 text-[11px] text-red-500">{editErrors.purchaseDate}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                    ...
                    />
                    {editErrors.tax && (
                      <p className="mt-1 text-[11px] text-red-500">{editErrors.tax}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditState((s) => ({ ...s, sourceDistrict: e.target.value }))
                      }
                      placeholder="e.g. Pune"
                      list="shopadmin-edit-source-district-options"
                      className={inputBase}
                    />
                  </div>
                  <div>
                    <datalist id="shopadmin-source-state-options">
                      {sourceStateOptions.map((state) => (
                        <option key={state} value={state} />
                      ))}
                    </datalist>
                    <datalist id="shopadmin-source-district-options">
                      {sourceDistrictOptions.map((district) => (
                        <option key={district} value={district} />
                      ))}
                    </datalist>
                    <datalist id="shopadmin-edit-source-district-options">
                      {editSourceDistrictOptions.map((district) => (
                        <option key={district} value={district} />
                      ))}
                    </datalist>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                    ...
                    />
                    {editErrors.saleGstRate && (
                      <p className="mt-1 text-[11px] text-red-500">{editErrors.saleGstRate}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                      Transportation Cost (₹) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                      ...
                      onChange={(e) =>
                        setEditState((s) => ({ ...s, transportationCost: e.target.value }))
                      }
                      className={`${inputBase} ${editErrors.transportationCost ? 'border-red-300' : ''}`}
                    />
                    {editErrors.transportationCost && (
                      <p className="mt-1 text-[11px] text-red-500">
                        {editErrors.transportationCost}
                      </p>
                    )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditState((s) => ({ ...s, lowStockAlertQty: e.target.value }))
                      }
                      placeholder={`Defaults to ${defaultLowStockThreshold}`}
                      className={`${inputBase} ${editErrors.lowStockAlertQty ? 'border-red-300' : ''}`}
                    />
                    {editErrors.lowStockAlertQty && (
                      <p className="mt-1 text-[11px] text-red-500">{editErrors.lowStockAlertQty}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="date"
                    value={editState.mfgDate}
                    onChange={(e) => setEditState((s) => ({ ...s, mfgDate: e.target.value }))}
                    className={inputBase}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditState((s) => ({ ...s, expiryDate: e.target.value }))}
